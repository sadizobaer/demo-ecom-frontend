...
export default function Header() {
  ...
  const { cartProductIds } = useUserData();
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 bg-[var(--surface)] border-b border-[var(--border)] ${
        scrolled ? "shadow-lg shadow-black/20 backdrop-blur-md bg-[var(--surface)]/95" : ""
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14 sm:h-16">
          ...
          {/* Logo */}
          <Link href="/" className="flex items-center gap-2 text-lg sm:text-xl font-bold flex-shrink-0">
            <span className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-[var(--accent)] flex items-center justify-center text-white text-xs sm:text-sm font-black">
              SW
            </span>
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <Link
              href="/cart"
              id="header-cart-btn"
              aria-label={cartCount ? `Cart — ${cartCount} item${cartCount !== 1 ? "s" : ""}` : "View Cart"}
            ...
            >
              ...
              {/* Count bubble */}
              {cartLabel && (
                <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] px-1 flex items-center justify-center rounded-full bg-[var(--accent)] text-white text-[10px] font-bold leading-none shadow-lg shadow-[var(--accent)]/40 animate-in zoom-in duration-200">
                  {cartLabel}
                </span>
              )}
            </Link>
              ...
              {isLoggedIn ? (
                <div className="relative">
                  ...
                  >
                    ...
                    <span className="text-sm font-medium text-[var(--text-primary)] max-w-[80px] truncate hidden md:block">
                  ...
                  {userMenuOpen && (
                    <div className="absolute right-0 top-full mt-2 w-48 bg-[var(--surface)] border border-[var(--border)] rounded-xl shadow-xl overflow-hidden z-50">
                      <Link href="/profile" className="flex items-center gap-2 px-4 py-2.5 text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-white/5 transition-all">
                        <svg className="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                        My Profile
                      </Link>
                      <Link href="/orders" className="flex items-center gap-2 px-4 py-2.5 text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-white/5 transition-all">
                        <svg className="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
                        My Orders
                      </Link>
                      <Link href="/wishlist" className="flex items-center gap-2 px-4 py-2.5 text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-white/5 transition-all">
                        <svg className="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z" /></svg>
                        Wishlist
                      </Link>
                      <Link href="/favorites" className="flex items-center gap-2 px-4 py-2.5 text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-white/5 transition-all">
                        <svg className="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
                        Favorites
                      </Link>
                ...
                <>
                  <Link id="header-login-btn" href="/login" className="px-3 py-1.5 rounded-lg text-sm font-medium text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors">
                    Login
                  </Link>
                  <Link id="header-register-btn" href="/register" className="px-3 py-1.5 rounded-lg text-sm font-semibold bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-white transition-colors">
                    Register
                  </Link>
                </>
              )}
            ...
            <button
              ...
              className="sm:hidden p-2 rounded-lg text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-white/5 transition-all"
            >
              {menuOpen ? (
      ...
      >
        <div className="bg-[var(--surface)] border-t border-[var(--border)] px-4 py-3 space-y-1">
          ...
          {/* Cart link with count in mobile menu */}
          <Link
            href="/cart"
            className={`flex items-center justify-between px-4 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ${
              pathname === "/cart"
          ...
          >
            <span>Cart</span>
            {cartLabel && (
              <span className="min-w-[20px] h-5 px-1.5 flex items-center justify-center rounded-full bg-[var(--accent)] text-white text-[10px] font-bold">
                {cartLabel}
              </span>
            )}
          </Link>
          ...
          <div className="border-t border-[var(--border)] pt-3 mt-2">
            {isLoggedIn ? (
              <div className="space-y-1">
                ...
                <Link href="/orders" className="block px-4 py-2.5 rounded-lg text-sm font-medium text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-white/5 transition-all">
                  My Orders
                </Link>
                <button
                  onClick={logout}
